...
const scoreByFilters = (item, filters, kind) => {
  ...
  if (kind === "driver") {
    const trainingCompletion = Number(item.trainingSummary?.completionRate || 0);
    const trainingScore = Number(item.trainingSummary?.avgScore || 0);

    score += trainingCompletion * 0.08;
    score += trainingScore * 0.05;

    if (filters.trainedOnly && Boolean(item.trainingSummary?.isTrained)) {
      score += 6;
    }

    if (filters.minTrainingScore && trainingScore >= Number(filters.minTrainingScore)) {
      score += 5;
    }
  }
...
const Marketplace = ({ currentRole }) => {
  ...
  const rankedDrivers = useMemo(() => {
    return [...drivers]
      ...
      .filter((driver) => {
        if (!filters.trainedOnly) return true;
        return Boolean(driver.trainingSummary?.isTrained);
      })
      .filter((driver) => {
        if (!filters.minTrainingScore) return true;
        return Number(driver.trainingSummary?.avgScore || 0) >= Number(filters.minTrainingScore);
      })
    ...
    <div style={pageStyle}>
      <div style={heroStyle}>
        <h2 style={{ margin: "10px 0 8px", fontSize: "34px" }}>
          Browse employers with ranked recommendations
        </h2>
        <p style={{ margin: 0, maxWidth: "760px", lineHeight: 1.6, opacity: 0.92 }}>
          Use location, rating, salary, and work-type filters to prioritize the best matches.
        </p>
      </div>
      ...
      <div style={{ ...panelStyle, marginBottom: "18px" }}>
        ...
        <div style={filterGrid}>
          ...
          />
          {showDrivers && (
            <label style={{ ...inputStyle, display: "flex", alignItems: "center", gap: "8px", borderStyle: "dashed" }}>
              <input type="checkbox" name="trainedOnly" checked={filters.trainedOnly} onChange={handleChange} />
              Trained drivers only
            </label>
          )}
          {showDrivers && (
            <input
              name="minTrainingScore"
              value={filters.minTrainingScore}
              onChange={handleChange}
              type="number"
              min="0"
              max="100"
              placeholder="Minimum training score"
              style={inputStyle}
            />
          )}
            ...
            return (
              <article key={driver._id} style={cardStyle}>
                ...
                <div style={metaStyle}>
                  ...
                  <span style={badgeStyle}>Training score: {Number(driver.trainingSummary?.avgScore || 0)}</span>
                  <span style={badgeStyle}>{driver.trainingSummary?.isTrained ? "Trained" : "In training"}</span>
                </div>
